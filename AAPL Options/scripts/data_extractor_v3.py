...
class DataExtractor():
    def __init__(self,symbool = 'SP',callput ='c',interpolator ="linear",test = False,data_path = 'AAPL_master_data.csv',
                 rates_path = 'RiskFreeRates.csv',spot_path = 'AAPL_spot.csv',db = True,sp_exp = True,delta = None,mn = None,expiry =12):
       self.test = test
       self.data_path = data_path
       self.db = db
       self.sp_exp = sp_exp
       self.exists = 0
       self.optkey = self.gen_key(delta,mn,callput,symbool,expiry)
      
       
       if db:
        
        exists = check_key(self.optkey)
        self.exists = exists

        if exists:
            self.df = extract_from_db(self.optkey)
            conn, cursor = get_db()
            ...
            self.df,self.rates, self.spot_data = load_full_data(callput,symbool)
       ...
       self.dates = np.sort(self.df.TradeDate.unique())
    def gen_key(self,delta,mn, callput,symbol,expiry):
        if delta:
            var = 'delta'
            val = delta
        else:
            var = 'mn'
            val = mn
        return f'{var}_{val}_{callput}_{symbol}_{expiry}'
    def get_days(self,date,months,symbool,s1,returned_dates = []):
         
       
        '''
        date: trade date
        months : expected months to exp
        s1 : sorted expiry dates of the available options on the above trade date
        '''
        # fl = False
        last_itr = False
        ...
        exp = pd.Series([dt for dt in s1 if dt not in returned_dates])
        ...
        if len(exp)==1 :
            ...
            last_itr= True
        # elif len(returned_dates)>0 and fl:
        #     last_itr= True
    ...
    def get_params_by_trade_date(self,date,callput,expiry,symbool):
            ...
            while not last_itr:
                ...
                for stk in available_strikes:
                    ...
                    exp_data = self.df[(self.df.TradeDate==date)&(self.df.AdjExpiry==exp_date)]
    ...
    def backfill(self,date_group_dt,df,date,callput,bydelta,delta,moneyness,band,symbool,expiry_in_months):
        ...
        exp_dates = date_group_dt.AdjExpiry.unique()
        exp_dates = pd.to_datetime(exp_dates,format="%Y%m%d")
        trade_date = pd.to_datetime(date, format="%Y%m%d")
        if symbool == 'EV':
            l_lim = trade_date + DateOffset(months=expiry_in_months)
            l_lim = l_lim.replace(day=1)    #+pd.offsets.Week(2)+pd.offsets.Week(weekday=2)
            
            u_lim = trade_date + DateOffset(months=expiry_in_months+1)
            u_lim = u_lim + pd.offsets.MonthEnd(0)
        elif symbool == 'SP':
            l_lim = trade_date + DateOffset(months=expiry_in_months)
            l_lim = l_lim.replace(day=1)    #+pd.offsets.Week(2)+pd.offsets.Week(weekday=2)
            u_lim = trade_date + DateOffset(months=expiry_in_months)
            u_lim = u_lim+ pd.offsets.MonthEnd(0)   # replace(day=1)+pd.offsets.Week(2)+pd.offsets.Week(weekday=5)
        exp_dates = exp_dates[(exp_dates > l_lim)&(exp_dates<u_lim)]
        if exp_dates.empty:
            exp_dates =df.AdjExpiry.unique()
            # s3 = [dt for dt in s3 if self.is_third_week_expiry(dt)]
            exp_dates = pd.to_datetime(exp_dates,format="%Y%m%d")
            exp_dates = exp_dates[(exp_dates > l_lim)&(exp_dates<u_lim)]
        if exp_dates.empty:
            exp_date = trade_date + DateOffset(months=expiry_in_months)
            exp_date = exp_date.replace(day=1)+pd.offsets.Week(2)+pd.offsets.Week(weekday=4)
            date_list = pd.to_datetime(self.spot_data.TradeDate,format='%Y%m%d')
            if exp_date<date_list.iloc[-1]:
                if exp_date not in pd.to_datetime(self.spot_data.TradeDate,format='%Y%m%d'):
                    raise ValueError(f'no trade, for date:{exp_dates}') 
            exp_dates = [exp_date]
        ...
        while not cond_satisfied:
            ...
            days_to_expiry,exp_date,last_itr = self.get_days(date,expiry_in_months,symbool,exp_dates,returned_dates=returned_dates)
    ...
    def extract_data(self,expiry_in_months = 3,moneyness = 90,delta = None,bydelta = False,callput = 'c',hold=1,bot=1,symbool = 'EV',start_date = None,end_date= None):
        ...
        if start_date:
            ...
            start_idx = np.where(self.dates==dates[0])[0][0]
        else:
            start_idx=0
        if end_date:
        ...
        df  = self.df
        if self.sp_exp:
            Exp_DtTime = pd.to_datetime(df["AdjExpiry"],format="%Y%m%d")
            is_third_week = Exp_DtTime.dt.day.between(15, 22)
            df = df[is_third_week] 
        # print(df.columns)
        ...
        for i in range(0,len(dates),bot):
            ...
            if start_idx+i+expiry_in_months<=len(self.dates)-1:
                last_trade = self.dates[start_idx+i+expiry_in_months]
            else:
                ...
                date_tup = last_trade.date().isoformat().split('-')
                last_trade = ''
                for j in date_tup:
                    last_trade+=j
            ...
            # print(date,last_trade,current_df.empty)
            # print(current_df.AdjExpiry.unique,current_df.Delta.unique)
            # current_df = current_df[current_df.AdjExpiry.map(self.is_third_week_expiry)]
            m1 = (current_df.Month_To_Expiry==expiry_in_months+1)
            m2 = (current_df.Month_To_Expiry==expiry_in_months)
           
            if symbool=='EV':
                if not current_df[(current_df.AdjExpiry>=last_trade)&m1].empty:
                    df_trade = current_df[(current_df.AdjExpiry>=last_trade)&m1]
                    df_trade = self.check_moneyness(df_trade, var,val)
                    # days_valid = df_trade.days_to_expiry.min()
                    flag = False

                elif not current_df[m2&(current_df.AdjExpiry<last_trade)].empty :
                    
                    df_trade = current_df[m2&(current_df.AdjExpiry<last_trade)]
                ...
                # if not current_df[(current_df.Month_To_Expiry==expiry_in_months)].empty:
                #     df_trade = current_df[(current_df.Month_To_Expiry==expiry_in_months)]
                #     df_trade = self.check_moneyness(df_trade, var,val)

                #     # days_valid = df_trade.days_to_expiry.max()
                #     flag = False

                if not current_df[(current_df.AdjExpiry>=last_trade)&(current_df.Month_To_Expiry==expiry_in_months)].empty:
                    df_trade = current_df[(current_df.AdjExpiry>=last_trade)&(current_df.Month_To_Expiry==expiry_in_months)]
                    df_trade = self.check_moneyness(df_trade, var,val)
                    # days_valid = df_trade.days_to_expiry.min()
                    flag = False
        
                
                elif not current_df[(current_df.Month_To_Expiry==expiry_in_months)&(current_df.AdjExpiry<last_trade)].empty:
                    df_trade = current_df[(current_df.Month_To_Expiry==expiry_in_months)&(current_df.AdjExpiry<last_trade)]
    ...
    def hold_options(self,data,hold,last_date,count,var):
        ...
        idx = np.where(self.dates==data['TradeDate'].values)[0][0]
        
        
        for j in range(hold+1):
            if self.dates[idx+j]>data.AdjExpiry.values[0]:
                break
            ...
            if df_iv.empty :
                ...
                flag = 1
                count +=1
            elif self.dates[idx+j] not in df_iv.TradeDate.to_list():
                # print(data.TradeDate.values[0],dates[idx+j],data.AdjExpiry.values[0],data.AdjStrike.values[0],data.Strike.values[0])
                # replace ExpiryDate with AdjExpiry next two
                # print(data.Strike.values[0],data.AdjStrike.values[0])
                if self.dates[idx+j]< data.AdjExpiry.values[0]:
                    px,delta,gama,vega,theta,rho,rfr,imp_vol = self.get_premium(data.CallPut.values[0],data.AdjStrike.values[0],self.dates[idx+j],data.AdjExpiry.values[0])
                    flag =1
                    count+=1
                ...
                df_opt = df_iv[df_iv.TradeDate == self.dates[idx+j]]#.iloc[[0]]
    ...
    def load_start_data(self,hold=1,bot=1,start_date = None,end_date= None,callput = 'c',symbool ='SP',expiry_in_months =12,delta = None,moneyness = None,bydelta = False):
        if self.exists:
            df_opt = extract_start_data(self.optkey)
        else:                

            df_opt = self.extract_data(expiry_in_months = expiry_in_months,moneyness = moneyness,delta = delta,bydelta = bydelta,callput = callput,hold=hold,bot=bot,symbool =symbool,start_date = None,end_date= None)
        if start_date:
            df_opt = df_opt[df_opt.TradeDate>=start_date]
        if end_date:
            df_opt = df_opt[df_opt.TradeDate<end_date]
        return df_opt
    ...
    def gen_data(self,moneyness = 100,delta=None,bydelta = False, time_to_expiry = 1,callput = 'p',bot =1,hold=1,symbool='EV',start_date = None,end_date= None,multiprocess = False,backfill=False):
        ...
        if not end_date:
            ...
            last_date = self.dates[self.dates<= end_date].max()
        option_accumulator = []
        missing = []
        count = 0
