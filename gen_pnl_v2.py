...
def compute_contracts(x,df_cnt,capital):
    ...
    return contracts
def stock_pnl(data_st,data_end,dh,stock,iv):
    days = 365
    stock_start = data_st.AdjSpot.values[0]
    stock_end = data_end.AdjSpot.values[0]
    if data_end.TradeDate.values[0]<= data_st.ExpiryDate.values[0]:
        if dh =='inception':
            pnl_stock = stock*(stock_end-stock_start)
        elif dh =='inception_vol':
            implied_vol = iv
            delta_end = delta_computation(data_end.CallPut,data_end.AdjSpot,data_end.AdjStrike,data_end.days_to_expiry/days,data_end.risk_free_rate,iv)
            delta_start = data_st.Delta
            stock = -stock_start.Delta
            pnl_stock = stock*(stock_end-stock_start)

        elif dh =='market_vol':
            delta_start = data_st.Delta
            delta_end = data_end.Delta
            stock = -stock_start.Delta
            pnl_stock = stock*(stock_end-stock_start)
...
def compute_pnl(data,mt,bot_on,spot,dh, stock = None,iv= None):
    multiplyer = 1   
    ...
    if dh!= 'off':
        ...
        pnl_stock = stock_pnl(data_st,data_end,dh,stock,iv)
    ...
    if dh!= 'off':
        data_st[dh] =pnl+pnl_stock/px_start
...
def gen_options_pnl(data_path = 'AAPL_master_data.csv',rates_path = 'RiskFreeRates.csv',moneyness = 100,delta = None,bydelta=False, time_to_expiry = 1,callput = 'p',bot =1,hold = 1, buysell = 'buy',capital = 10000000,
    ...
    for i in range(df_opt.shape[0]-1):
        ...
        if data.AdjStrike.iloc[0]==data.AdjStrike.iloc[1] and data.AdjExpiry.iloc[0]==data.AdjExpiry.iloc[1] and data.TradeDate.iloc[0]<data.TradeDate.iloc[1]:
            if flag ==0:
            ...
            pnl_frame = compute_pnl(data,mt,bot_on,spot,dh,stock = stock,iv=iv)
    ...
    if dh!='off':
        df_opt[['Flag','BotOn', 'TradeDate', 'ExpiryDate', 'CallPut', 'AdjStrike', 'AdjSpot',
                     'ExpirySpot','Symbol',  'OptPxBot', 'OptPxTrade', 'SpotPct','Delta',
                     'Strike', 'SpotStart','OptPnl',dh, 'TradeStart']]
