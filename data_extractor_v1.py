...
class DataExtractor():
    def __init__(self,interpolator ="linear",test = False,data_path = 'AAPL_master_data.csv',rates_path = 'RiskFreeRates.csv'):
       ...
       # self.test_data = []
    def get_days(self,date,months,s1,s2,returned_dates = []):
        ...
        l_lim = trade_date + DateOffset(months=months)-pd.Timedelta(days=15)
        
        u_lim = trade_date + DateOffset(months=months+1)-pd.Timedelta(days=15)
        
        exp = s1[(s1 >= l_lim)&(s1<u_lim)]
        exp = pd.Series([dt for dt in exp if dt not in returned_dates])
        if len(exp==1):
            exp_test = s2[(s2 >= l_lim)&(s2<u_lim)]
            exp_test= pd.Series([dt for dt in exp_test if dt not in returned_dates])
            exp_test == exp.min()
            last_itr= True
    ...
    def check_moneyness(self,df_trade, var,val):
        ...
        return df_trade
    def gen_iv_surface(self,date,callput):
        # extract data based on trade date and callput & clean ang gen iv surface
        data = self.df[(self.df.TradeDate==date)&(self.df.CallPut==callput)].copy()
    ...
    def extract_data(self,expiry_in_months = 3,moneyness = 90,delta = None,bydelta = False,callput = 'c',hold=1,bot=1,symbool = 'EV',start_date = None,end_date= None):
        ...
        for i in range(0,len(dates),bot):
            ...
            if symbool=='EV' and not current_df[(current_df.days_to_expiry>=days)&(self.df.Month_To_Expiry==expiry_in_months)].empty:
                df_trade = current_df[(current_df.days_to_expiry>=days)&((self.df.Month_To_Expiry==expiry_in_months))]
                df_trade = self.check_moneyness(df_trade, var,val)
                days_valid = df_trade.days_to_expiry.min()
                
            elif symbool=='EV' and not current_df[(current_df.days_to_expiry>=days)&(self.df.Month_To_Expiry==expiry_in_months+1)&
                                                  (pd.to_datetime(current_df.AdjExpiry,format="%Y%m%d").dt.day<=15)].empty:
                df_trade = current_df[(current_df.days_to_expiry>=days)&(self.df.Month_To_Expiry==expiry_in_months+1)&
                                      (pd.to_datetime(current_df.AdjExpiry,format="%Y%m%d").dt.day<=15)]
                df_trade = self.check_moneyness(df_trade, var,val)
                days_valid = df_trade.days_to_expiry.min()
            elif symbool=='SP' and not current_df[(current_df.days_to_expiry>=days)&(self.df.Month_To_Expiry==expiry_in_months)].empty:
                df_trade = current_df[(current_df.days_to_expiry>=days)&(self.df.Month_To_Expiry==expiry_in_months)]
                df_trade = self.check_moneyness(df_trade, var,val)
                days_valid = df_trade.days_to_expiry.min()

            elif symbool=='EV' and not current_df[(self.df.Month_To_Expiry==expiry_in_months)&(current_df.days_to_expiry<days)&(pd.to_datetime(current_df.AdjExpiry,format="%Y%m%d").dt.day>=15)].empty :
                df_trade = current_df[(self.df.Month_To_Expiry==expiry_in_months)&(current_df.days_to_expiry<days)&(pd.to_datetime(current_df.AdjExpiry,format="%Y%m%d").dt.day>=15)]
                
                df_trade = self.check_moneyness(df_trade, var,val)
                days_valid = df_trade.days_to_expiry.max()
                
            elif symbool == 'SP' and not current_df[(self.df.Month_To_Expiry==expiry_in_months)&(current_df.days_to_expiry<days)].empty:
                df_trade = current_df[(self.df.Month_To_Expiry==expiry_in_months)&(current_df.days_to_expiry<days)]
                ...
                surface_genrator,exp_dates,y = self.gen_iv_surface(dates[i],callput)
                ...
                while not cond_satisfied:
                    days_to_expiry,exp_date,last_itr = self.get_days(date,expiry_in_months,pd.to_datetime(exp_dates,format="%Y%m%d").unique(), pd.to_datetime(self.df.AdjExpiry,format="%Y%m%d").unique(),returned_dates=returned_dates)
    ...
    def hold_options(self,data,hold,last_date,count,var):
        ...
        rec_df = pd.DataFrame({'TradeDate':[data.TradeDate.values[0]], 'Expiry':[data.ExpiryDate.values[0]],'C/P': data.CallPut.values[0],var:data[var].values[0],
