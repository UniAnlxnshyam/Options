...
def compute_pnl(data,mt,bot_on,spot,px_trade_start,multiplyer):
...
def gen_options_pnl(data_path = 'AAPL_master_data.csv',rates_path = 'RiskFreeRates.csv',moneyness = 100,delta = None,bydelta=False, time_to_expiry = 1,callput = 'p',bot =1,hold = 1, buysell = 'buy',
    ...
    df_opt = df_opt[['TradeDate','AdjExpiry', 'AdjStrike',
                     ...
                     'CallPut', 'Symbol','Flag','TradeStart']]
    if not end_date:
    ...
    for k,v in date_group.items():
        ...
        if v.shape[0]==1 and k== last_trade :
            ...
            for i in range(v.shape[0]-1):  
                ...
                data = v.iloc[i:i+2]        
                pnl_frame = compute_pnl(data,mt,k,spot_start,px_trade_start,multiplyer)                  
    ...
    df_op = df_opt[['Flag','BotOn', 'TradeDate', 'AdjExpiry', 'Strike', 'AdjSpot',
...
def wealth_computation(df,sizing ='notional',pct = 1,num_contracts =1000,contract_size = 1,bot =1,hold=1,init_cap = 1000000):
    ...
    # elif sizing =='fixed_start_wealth':
    #      opt_df['current_cap'] = (capital/opt_df.AdjSpot.iloc[0])*opt_df.SpotStart
    #      opt_df['NumContracts'] = opt_df['current_cap']/opt_df['SpotStart']
